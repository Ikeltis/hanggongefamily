 ...
 *   3. 结果只推送一条通知。
 *
 * 定时：本脚本**刻意不声明 `cron:`**，因为开抢时刻取决于当期活动，没有通用值，
 *       写死一个反而会让人以为它是对的。请自己在面板里设。
 *
 *       设法：把任务排在开抢时刻「前 1~2 分钟」触发，脚本内部自旋等待到点开抢。
 *       例如 12:00 开抢 → cron 设 `58 11 * * *`，并设 HZGH_EXCHANGE_TIME=12:00:00
 *
 *       ⚠️ 订阅自动建任务时，面板对没有 `cron:` 声明的脚本会塞一个默认值
 *       `0 0 * * *`（每天零点）。**记得进面板把它清空或改掉**，否则会在
 *       毫无意义的时间点空跑。
 *
