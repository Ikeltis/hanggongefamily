...
**多账号**：两个变量都用 `&` 分隔（换行也行），按位置一一对应。每个账号各跑一次
取码工具，把结果拼起来：
...
只填一个值就是单账号，和以前完全一样，老配置不用改。

两边数量对不上会**直接报错退出**——不这么做的话会把甲的 `login_name` 和乙的
`ses_id` 配成一个账号，请求照样发得出去，但结果是错的且极难排查。

签到按账号依次执行，单个账号失败不影响其他账号；抢券是掐点的，所有账号**同时**
开抢（串行的话第二个账号要等第一个的窗口跑完，整场就错过了）。其余参数（签到
次数、评论内容、券类型…）全账号共用。
